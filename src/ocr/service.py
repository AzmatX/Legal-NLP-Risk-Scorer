...
class OptimizedOCRProcessor:
    def __init__(self, preferred_engine: str = "auto", max_workers: int = 4, dpi: int = 300, use_cache: bool = True):
        self.preferred_engine = preferred_engine
        self.max_workers = max_workers
        self.dpi = dpi
        self.use_cache = use_cache

        # Engine availability (✅ FIX: Proper try-except)
        self.tesseract_available = False
        self.easyocr_available = False
        self.paddleocr_available = False
        self.pdf2image_available = False

        # Singleton engine instances (✅ FIX: Lazy loading to avoid reloading per page)
    ...
    def _detect_engines(self):
        ...
        except: logger.info("❌ Tesseract not available")

        try:
            import easyocr
            self.easyocr_available = True
            logger.info("✅ EasyOCR available")
        except: logger.info("❌ EasyOCR not available")

        try:
            import paddleocr
            self.paddleocr_available = True
            logger.info("✅ PaddleOCR available")
        except: logger.info("❌ PaddleOCR not available")

        try:
            import pdf2image
            self.pdf2image_available = True
            logger.info("✅ pdf2image available")
        except: logger.info("❌ pdf2image not available")
    ...
    def _select_engine(self) -> str:
        ...
        if self.paddleocr_available: return "paddleocr"
        if self.easyocr_available: return "easyocr"
        if self.tesseract_available: return "tesseract"
        return "none"

    def extract_text_from_document(self, file_source, is_file_path=True, use_parallel=True) -> dict:
        start_time = time.time()
        result = {"full_text": "", "pages": [], "page_count": 0, "success": False, "error": None, "engine_used": "none"}

        try:
            if is_file_path:
                path = Path(file_source)
                if not path.exists(): raise FileNotFoundError(f"File not found: {file_source}")
            ...
            if self.use_cache: ocr_cache.set(raw_bytes, result)
            return result
    ...
    def _process_pages_parallel(self, images, engine):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as ex:
            futures = {ex.submit(self._process_single_page, img, engine, i): i for i, img in enumerate(images)}
    ...
    def _process_single_page(self, image, engine, page_idx=0):
        if engine == "tesseract":
            ocr = self._get_tesseract()
            if ocr: return ocr.image_to_string(image, config="--oem 3 --psm 6").strip()
        elif engine == "easyocr":
...
def extract_text_from_document(file_source, is_file_path=True, dpi=300):
    p = _get_default_processor()
    p.dpi = dpi
    return p.extract_text_from_document(file_source, is_file_path)

def clear_ocr_cache(): return ocr_cache.clear()